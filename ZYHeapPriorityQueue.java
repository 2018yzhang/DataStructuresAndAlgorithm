 ...
 * @version 11/5/2016
 */
public class ZYHeapPriorityQueue<ElementType extends Comparable<ElementType>> implements ZHQueue<ElementType> {
   ...
   */
  public ZYHeapPriorityQueue(int capacity) {
    if(capacity <= 0){
  ...
  protected int leastPosition (int currentPositon){
    if(leftChildPosition(currentPositon)>=size()){
      ...
      if(comp<=0){
        ...
        min=leftChildPosition(currentPositon);
      }
    if(rightChildPosition(currentPositon)<size()){
   
            int diff =elements.get(min).compareTo(elements.get(rightChildPosition(currentPositon)));
            if(diff<=0){
              return min;
            }
            else{
              min= rightChildPosition(currentPositon);
            }
      }
         return min;
    }
...
//    }
     ...
     if(leastPosition(currentPosition)==currentPosition){
     return;
     }
      else{
      int min = leastPosition(currentPosition);
      ElementType OldElt = elements.get(currentPosition);
        elements.set(currentPosition,elements.get(min));
        elements.set(min, OldElt);
         this.checkDownward(min);
      }
